...
function JobCard({
  ...
  return (
    <div
      className={`${styles.card} ${item.isExpired ? styles.cardExpired : ""}`}
    >
      {item.isExpired && (
      ...
      {!item.isExpired && item.daysLeft !== null && item.daysLeft <= 3 && (
        ...
        >
          <Clock size={11} /> Only{" "}
          {item.daysLeft === 0 ? "today" : `${item.daysLeft}d`} left to apply!
        </div>
      )}
      ...
      <div className={styles.cardTop}>
        ...
        <div className={styles.cardInfo}>
          ...
          <div className={styles.cardMeta}>
            <span>
              <Building2 size={10} /> {job.company.companyName}
            </span>
            <span>
          ...
          {item.matchedSkills.length > 0 && (
            <div className={styles.tagRow}>
              {item.matchedSkills.slice(0, 4).map((t) => (
                <span key={t} className={`${styles.tag} ${styles.tagMatch}`}>
                  {t}
                </span>
              ))}
              {job.skills
                .filter((s) => !item.matchedSkills.includes(s))
                .slice(0, 2)
                .map((t) => (
      ...
      <div className={styles.cardBottom}>
        <div className={styles.cardDates}>
          <span>
            <Bookmark size={10} /> Saved {timeAgo(item.savedAt)}
          </span>
          {job.expiresAt && (
            <span
              style={{
...
export default function SavedJobsPage() {
  ...
  return (
    <div className={styles.page}>
      ...
      {/* Error */}
      {error && (
        <p style={{ color: "var(--status-danger)", fontSize: 14 }}>{error}</p>
      )}
